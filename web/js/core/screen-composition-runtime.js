(function (root) {
    ...
    function orphanLegacyPresentation(screenRoot) {
        ...
        const dynamic = [...screenRoot.querySelectorAll(
            '[data-tq-dev-kind="dynamicText"], [data-tq-composition-dynamic]'
        )];
        const preserved = new Set(dynamic);
        const keep = new Set([screenRoot]);

        [...functions, ...dynamic].forEach((node) => {
            let current = node;
        ...
        [...screenRoot.querySelectorAll("*")].forEach((element) => {
            ...
            if (preserved.has(element)) return;
    ...
    function mount(options = {}) {
        ...
        const presentationRoot = options.presentationRoot instanceof Element
            ? options.presentationRoot
            : screenRoot;
        hideLegacyVisuals(presentationRoot);
        orphanLegacyPresentation(presentationRoot);
        ...
        function resolveHomeRuntimeBinding(slot, binding) {
            if (screenType !== "home") return binding;
            const avatarId = String(runtimeState?.player?.avatarId || "sofia");
            const avatarAsset = TQ.content?.assets?.avatars?.[avatarId]
                || TQ.content?.assets?.avatars?.sofia
                || binding?.asset || null;
            const heroAsset = TQ.content?.assets?.homeHeroes?.[avatarId]
                || TQ.content?.assets?.homeHeroes?.sofia
                || avatarAsset;
            const level = Math.min(10, Math.max(1, Number(runtimeState?.progression?.level) || 1));
            const frameId = String(runtimeState?.player?.frameId || "");
            const frame = TQ.content?.frames?.find?.((item) => item.id === frameId)
                || TQ.content?.frames?.find?.((item) => item.id === TQ.content?.defaultFrameId)
                || TQ.content?.frames?.[0] || null;
            if (slot.id === "home.header.frame" && frame?.asset) return { ...binding, asset: frame.asset };
            if (slot.id === "home.header.avatar") return { ...binding, asset: avatarAsset };
            if (slot.id === "home.header.level-plate") {
        ...
        function resolveBinding(slot, bindings) {
            ...
            const activeVariant = variants.find((variant) => variant.id === preferredId)
                ...
                || null;
            return resolveHomeRuntimeBinding(slot, {
                ...binding,
