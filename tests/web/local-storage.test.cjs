...
test("sem persistência carrega v13",()=>{const s=p.loadState(memory());assert.equal(s.schemaVersion,13);assert.equal(Object.keys(s.campaign.regionProgress).length,22)});
