...
test("sem persistência carrega v9",()=>{const s=p.loadState(memory());assert.equal(s.schemaVersion,9);assert.equal(Object.keys(s.campaign.regionProgress).length,22)});
...
test("loadState migra save v8 para posição global equivalente",()=>{const old=legacyV8();old.campaign.currentIslandId=6;old.campaign.regionProgress["1"].islandsCompleted=6;old.campaign.completedIslandIds=Array.from({length:6},(_,i)=>`region-1-island-${i+1}`);const st=memory({[p.STORAGE_KEY]:JSON.stringify(old)});const l=p.loadState(st);assert.equal(l.schemaVersion,9);assert.equal(l.campaign.currentRegionId,2);assert.equal(l.campaign.currentIslandId,1);assert.equal(l.campaign.regionProgress["1"].islandsCompleted,5);assert.equal(l.campaign.regionProgress["2"].islandsCompleted,1)});
...
test("JSON corrompido volta ao inicial",()=>{const s=p.loadState(memory({[p.STORAGE_KEY]:"{broken"}));assert.equal(s.schemaVersion,9);assert.equal(s.wallet.gems,0)});
