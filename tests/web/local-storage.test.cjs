...
function legacyV8(){
    ...
    return {
        ...
        campaign:{
            ...
            completedIslandIds:[],travelPlayedIslandIds:[],regionProgress:progress,
            ...
            petsRescuedIds:[],claimedChestIds:[],specialMaps:maps,diamonds:0
        },
...
test("sem persistência carrega v9",()=>{
    const s=p.loadState(memory());
    assert.equal(s.schemaVersion,9);
    assert.equal(Object.keys(s.campaign.regionProgress).length,22);
});

test("persiste HUD e personalização",()=>{
    const st=memory();
    let s=d.createInitialState();
    s.player.displayName="Alana";
    s.player.profileFrameId="tide-wheel";
    s.wallet.coins=64;
    p.saveState(st,s);
    const l=p.loadState(st);
    assert.equal(l.player.displayName,"Alana");
    assert.equal(l.player.profileFrameId,"tide-wheel");
    assert.equal(l.wallet.coins,64);
});

test("loadState migra save v8 para a posição global equivalente",()=>{
    const old=legacyV8();
    ...
    const st=memory({[p.STORAGE_KEY]:JSON.stringify(old)});
    const l=p.loadState(st);
    assert.equal(l.schemaVersion,9);
    assert.equal(l.campaign.currentRegionId,2);
    assert.equal(l.campaign.currentIslandId,1);
    assert.equal(l.campaign.regionProgress["1"].islandsCompleted,5);
    assert.equal(l.campaign.regionProgress["2"].islandsCompleted,1);
});

test("persiste arco final sem tocar ledger normal",()=>{
    const st=memory();
    let s=d.createInitialState();
    ...
    for(let islandId=1;islandId<=5;islandId++) s=d.completeIsland(s,22,islandId);
    s=d.claimFinalGrandChest(s);
    p.saveState(st,s);
    const l=p.loadState(st);
    assert.equal(l.campaign.finalJourney.finalMapFragments,9);
    assert.equal(l.campaign.finalJourney.finalGrandChestClaimed,true);
    assert.equal(l.campaign.claimedChestIds.length,0);
});

test("JSON corrompido volta ao inicial",()=>{
    const s=p.loadState(memory({[p.STORAGE_KEY]:"{broken"}));
    assert.equal(s.schemaVersion,9);
