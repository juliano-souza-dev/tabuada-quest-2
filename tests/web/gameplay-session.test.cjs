...
test("sessão usa o plano real 22x5", () => {
    let session = gameplay.createIslandSession(2, 1, "slice");
    let regionState = gameplay.createRegionState(2);
    ({ session, regionState } = gameplay.prepareNextChallenge(session, regionState));
    assert.equal(session.currentChallenge.regionId, 2);
    assert.equal(session.currentChallenge.islandId, 1);
    assert.equal(session.currentChallenge.globalIslandIndex, 6);
    assert.equal(session.currentChallenge.exposureType, scheduler.EXPOSURE_TYPE.PLANNED);
});

test("resposta errada agenda recovery sem consumir exposição extra", () => {
...
test("Ilha completa após 20 questões planejadas", () => {
    let session = gameplay.createIslandSession(22, 4, "full-island");
    let regionState = gameplay.createRegionState(22);
    ...
    while (session.phase !== "complete") {
        if (session.phase === "question") {
            ...
            const challenge = session.currentChallenge;
            ({ session, regionState } = gameplay.answerCurrentChallenge(session, regionState, challenge.table * challenge.multiplier));
        } else {
