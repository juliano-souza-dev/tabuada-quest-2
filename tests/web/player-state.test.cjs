...
test("estado inicial v6 tem 11 regiões e estado de aprendizagem",()=>{const s=d.createInitialState();assert.equal(s.schemaVersion,6);assert.deepEqual(s.campaign.unlockedRegionIds,[1]);assert.equal(Object.keys(s.campaign.regionProgress).length,11);assert.equal(s.learning.activeSession,null)});
test("migração v5 adiciona estado de aprendizagem",()=>{const b=d.createInitialState();const old={...b,schemaVersion:5};delete old.learning;const m=d.normalizeState(old);assert.equal(m.schemaVersion,6);assert.equal(m.learning.activeSession,null)});
test("migração v4 adiciona campanha e aprendizagem",()=>{const b=d.createInitialState();const old={...b,schemaVersion:4,campaign:{currentRegionId:1,currentIslandId:1,unlockedRegionIds:[1],completedIslandIds:[],petsRescuedIds:[],claimedChestIds:[],specialMaps:b.campaign.specialMaps,diamonds:0}};delete old.learning;const m=d.normalizeState(old);assert.equal(m.schemaVersion,6);assert.equal(m.campaign.regionProgress["11"].islandsTotal,10)});
