...
test("estado inicial v8 tem 11 regiões, aprendizagem e Tripulação",()=>{const s=d.createInitialState();assert.equal(s.schemaVersion,8);assert.deepEqual(s.campaign.unlockedRegionIds,[1]);assert.equal(Object.keys(s.campaign.regionProgress).length,11);assert.equal(s.learning.activeSession,null)});
test("migração v5 adiciona estado de aprendizagem",()=>{const b=d.createInitialState();const old={...b,schemaVersion:5};delete old.learning;const m=d.normalizeState(old);assert.equal(m.schemaVersion,8);assert.equal(m.learning.activeSession,null)});
test("migração v4 adiciona campanha e aprendizagem",()=>{const b=d.createInitialState();const old={...b,schemaVersion:4,campaign:{currentRegionId:1,currentIslandId:1,unlockedRegionIds:[1],completedIslandIds:[],petsRescuedIds:[],claimedChestIds:[],specialMaps:b.campaign.specialMaps,diamonds:0}};delete old.learning;const m=d.normalizeState(old);assert.equal(m.schemaVersion,8);assert.equal(m.campaign.regionProgress["11"].islandsTotal,10)});
...
test("estado inválido volta ao inicial",()=>{const s=d.normalizeState({schemaVersion:999,wallet:{coins:999}});assert.equal(s.schemaVersion,8);assert.equal(s.wallet.coins,0)});
